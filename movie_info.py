        ...
        while tries < 2:
            ...
            if 'Search' in results.keys():
                ...
                if filtered:
                    ...
                    diff_movi_name = difflib.SequenceMatcher(None, selected['Title'], movi_name).ratio()
                    diff_querySearch = difflib.SequenceMatcher(None, selected['Title'], querySearch['s']).ratio()
                    if diff_movi_name >= 0.9 or (diff_querySearch >= 0.9 and diff_movi_name >= 0.8):
                        found = True
            ...
            search = movi_name
            search = ' '.join(word for word in search.split() if word not in STOPWORDS_EN)
            search = ' '.join(word for word in search.split() if word not in SPECIALCHARS)
